...
def generate_features(image_path):
    ...
    # Extract LBP features (Texture) - Fixed length
    radius = 1
    n_points = 8 * radius
    lbp = local_binary_pattern(gray_image, P=n_points, R=radius, method="default")
    lbp_hist, _ = np.histogram(lbp.ravel(), bins=128, range=(0, 255))
    lbp_hist = lbp_hist.astype(np.float32) / np.sum(lbp_hist)  # Normalize
    ...
    features = {
        ...
        "TextureFeatures": lbp_hist.tolist(),
